...
const validateCriticalServices = async () => {
    ...
    if (authOk && dbOk && appOk) {
        ...
        if (!appOk)
            console.error('Application code health check failed.');
        process.exit(1);
    }
};
// Root route
...
io.on('connection', (socket) => {
    ...
    // Call request
    socket.on(SocketsEvents.CALL_REQUEST, async (data) => {
        // Busy user check
        ...
        try {
            ...
            if (recipient?.expoPushToken) {
                await sendExpoNotification(recipient.expoPushToken, 'Incoming Call', `${data.from} is calling you!`, { callId: call._id, type: data.type });
            }
    ...
    // Call accept
    socket.on(SocketsEvents.CALL_ACCEPT, async (data) => {
    ...
    // Call reject
    socket.on(SocketsEvents.CALL_REJECT, async (data) => {
        if (!data ||
            typeof data.callId !== 'string' ||
            typeof data.to !== 'string' ||
            typeof data.from !== 'string') {
    ...
    // Call end
    socket.on(SocketsEvents.CALL_ENDED, async (data) => {
        if (!data ||
            typeof data.callId !== 'string' ||
            typeof data.to !== 'string' ||
            typeof data.from !== 'string') {
    ...
    socket.on(SocketsEvents.CHAT_MESSAGE, async (data) => {
        ...
        const recipient = await User.findById(data.to);
        if (recipient?.expoPushToken) {
            await sendExpoNotification(recipient.expoPushToken, 'New Message', `${data.from}: ${data.content}`, { messageId: message._id, from: data.from });
        }
    });
    // Start Typing event
    socket.on(SocketsEvents.CHAT_TYPING, (data) => {
    ...
    // Stop Typing event
    socket.on(SocketsEvents.CHAT_STOP_TYPING, (data) => {
        if (!data ||
            typeof data.to !== 'string' ||
            typeof data.from !== 'string') {
    ...
    // Message read event
    socket.on(SocketsEvents.MESSAGE_READ, async (data) => {
